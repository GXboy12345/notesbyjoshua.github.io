...
Unlike AP Physics C or F=ma, USAPhO leans heavily on estimation and other mathematical tools to solve problems. This page collects the recurring mathematical machinery — integrals, approximations, vectors, complex numbers, and differential equations — that you are expected to deploy almost reflexively. None of it is hard once seen a few times, but on a timed exam the difference between knowing $$\int_0^\infty e^{-x^2}\,dx$$ cold and re-deriving it is the difference between solving a problem and not.
...
$$
\int_0^\infty \frac{x^3}{e^x-1}\,dx=\frac{\pi^4}{15}\quad(\text{used in deriving the Stefan–Boltzmann law}).
$$
...
Often, for small values, you can take an approximation that will simplify the calculations a lot. For $$|x|\ll 1$$:
...
**Example (why every stable equilibrium oscillates like a spring).** Take any potential $$U(x)$$ with a stable equilibrium at $$x_0$$. Expand about $$x_0$$:
...
A vector $$\vec a$$ has components $$\vec a=(a_x,a_y,a_z)=a_x\hat i+a_y\hat j+a_z\hat k$$, where $$\hat i,\hat j,\hat k$$ are unit vectors along the axes. Its **magnitude** is $$|\vec a|=\sqrt{a_x^2+a_y^2+a_z^2}$$, and the **unit vector** in its direction is $$\hat a=\vec a/|\vec a|$$. Throughout, $$\theta$$ denotes the angle between the two vectors being combined.
...
1. $$|\vec a\times\vec b|=|\vec a||\vec b|\sin\theta$$ — the magnitude equals the area of the parallelogram spanned by the two vectors.
...
## Differential equations you will actually see
